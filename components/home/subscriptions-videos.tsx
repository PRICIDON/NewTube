...
export default function SubscriptionsVideosSection() {
    return (
...
function SubscriptionsVideosSectionSuspense() {
    const [videos, query] = trpc.videos.getSubscribed.useSuspenseInfiniteQuery({  limit: DEFAULT_LIMIT}, { getNextPageParam: lastPage => lastPage.nextCursor })
...
function SubscriptionsVideosSectionSkeleton() {
    return (
        <div
            className="gap-4 gap-y-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-4 [@media(min-width:1920px]:grid-cols-5 [@media(min-width:2200px]:grid-cols-6"
        >
